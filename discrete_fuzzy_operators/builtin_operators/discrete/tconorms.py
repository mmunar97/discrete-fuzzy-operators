...
class TconormExamples(Enum):
    ...
    LUKASIEWICZ_ORDINAL_SUM = "lukasiewicz_ordinal_sum_tconorm"

    @staticmethod
    def get_tconorm(tconorm: "TconormExamples", n: int, **kwargs) -> Tconorm:
        """
        ...
        elif tconorm == TconormExamples.LUKASIEWICZ_ORDINAL_SUM:
            if not "idempotent_elements" in kwargs:
                raise Exception("The idempotent elements must be provided to define a t-conorm generated by Lukasiewicz ordinal sums.")
            idempotent_elements = kwargs.get("idempotent_elements")
            return Tconorm(n=n, operator_expression=partial(TconormExamples.__lukasiewicz_ordinal_sum, idempotent_elements=idempotent_elements))
    ...
    @staticmethod
    def __lukasiewicz_ordinal_sum(x: int, y: int, n: int, idempotent_elements: List[int]):
        pass
